    ...
    try {{
        ...
        if (loginFormVisible) {{
            ...
            if (captchaFlag === 'True' || captchaFlag === 'true') {{
                console.log('CAPTCHA ENABLED — user must solve manually');
            ...
            // Wait for redirect back to Housing Connect
            try {{
                await page.waitForURL(/.*housingconnect\.nyc\.gov.*/, {{ timeout: 30000 }});
                console.log('Login successful! URL: ' + page.url());
                console.log('RESULT: LOGIN_OK');
            }} catch(e) {{
                console.log('Login redirect timeout — may have failed');
                await page.screenshot({{ path: shotDir + 'login_error.png' }}).catch(() => {{}});
                console.log('RESULT: LOGIN_FAILED');
            }}
        }} else {{
            // Maybe already logged in — check dashboard
            console.log('No login form — checking if already logged in...');
            ...
            if (!page.url().includes('unauthorized')) {{
                ...
                console.log('Not logged in and no login form found');
                console.log('RESULT: LOGIN_FAILED');
            }}
        }}
    }} catch(e) {{
...
(async () => {{
    ...
    try {{
        ...
        // Click Apply Now (it's an <a> tag, NOT a button)
        ...
        if (!applyVisible) {{
            console.log('Apply Now not found — lottery may be closed');
        ...
        for (const inp of inputs.slice(0, 40)) {{
            try {{
                ...
                if (visible) {{
                    visibleInputs.push({{ tag, type, name, id, placeholder }});
                    console.log('  INPUT ' + tag + ' type=' + type + ' name=' + name + ' id=' + id + ' placeholder=' + placeholder);
                }}
            }} catch(e) {{}}
        }}
        ...
        // These selectors will be updated once we inspect the logged-in form
        console.log('\\nAttempting to fill form fields...');
        
        // Try common field selectors for Angular Material forms
        const fillResults = [];
        
        // First name patterns
        for (const sel of ['input[name="firstName"]', '#firstName', 'input[placeholder*="First"]', 'input[aria-label*="First"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('first_name', ''))})) {{
                fillResults.push('first_name: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="lastName"]', '#lastName', 'input[placeholder*="Last"]', 'input[aria-label*="Last"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('last_name', ''))})) {{
                fillResults.push('last_name: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[type="email"]', 'input[name="email"]', '#email', 'input[placeholder*="Email"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('email', ''))})) {{
                fillResults.push('email: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[type="tel"]', 'input[name="phone"]', '#phone', 'input[placeholder*="Phone"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('phone', ''))})) {{
                fillResults.push('phone: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="dob"]', '#dob', 'input[placeholder*="birth"]', 'input[placeholder*="MM/DD/YYYY"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('dob', '').replace('-', '/'))})) {{
                fillResults.push('dob: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="ssn"]', '#ssn', 'input[placeholder*="SSN"]', 'input[placeholder*="XXX-XX"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('ssn', ''))})) {{
                fillResults.push('ssn: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="address"]', '#address', 'input[placeholder*="Address"]', 'input[placeholder*="Street"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('address', ''))})) {{
                fillResults.push('address: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="city"]', '#city', 'input[placeholder*="City"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('city', ''))})) {{
                fillResults.push('city: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="zip"]', '#zip', 'input[placeholder*="Zip"]', 'input[placeholder*="Postal"]']) {{
            if (await safeFill(sel, {json.dumps(primary.get('zip', ''))})) {{
                fillResults.push('zip: ' + sel);
                break;
            }}
        }}
        ...
        for (const sel of ['input[name="income"]', '#income', 'input[placeholder*="Income"]', 'input[placeholder*="0.00"]']) {{
            if (await safeFill(sel, str(income.get('annual_income', 0)))) {{
                fillResults.push('income: ' + sel);
                break;
            }}
        }}
        ...
        // DO NOT SUBMIT — wait for user review
...
def main():
    ...
    try:
        ...
        if 'CAPTCHA_NEEDED' in output:
            print('\n*** CAPTCHA DETECTED ***')
            print('Solve the captcha in Chrome, then press ENTER to continue...')
            input()
